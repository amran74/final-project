# Inventory.py — clean + fully working
...
# ---- custom inventory helpers ----
def add_item(user_id, name, expiration, food_type, amount, unit, stable=False, price_per_unit=0.0):
    conn = get_connection(); c = conn.cursor()
    c.execute("""
        INSERT INTO inventory
        (user_id, name, expiration, type, amount, unit, used_count,
         last_used_month, stable, price_per_unit, expired_count, money_lost)
        VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, 0, 0.0)
    """, (user_id, name, expiration, food_type, amount, unit,
          date.today().strftime("%Y-%m"), int(stable), price_per_unit))
    conn.commit(); conn.close()

def get_user_items(user_id):
    conn = get_connection(); c = conn.cursor()
    c.execute("""
        SELECT id, name, expiration, type, amount, unit,
               used_count, last_used_month, stable, price_per_unit,
...
def db_update_item(item_id, name, expiration, food_type, amount, unit, price_per_unit):
    conn = get_connection(); c = conn.cursor()
    c.execute("""
        UPDATE inventory
...
def inventory():
    ...
    with st.form("add_item", clear_on_submit=True):
        ...
        if st.form_submit_button("Add"):
            if not name.strip(): st.error("Name required.")
                ...
                add_item(user_id, name.strip(), exp.strftime("%Y-%m-%d"), typ,
                         float(amount), unit, stable, float(ppu))
    ...
    for i, r in enumerate(rows):
        (item_id, name, exp, typ, amount, unit,
         used_count, last_m, stable, ppu,
         expired_steps, money_lost) = r
        dleft = (datetime.strptime(exp, "%Y-%m-%d").date() - date.today()).days
        status, color = ("Expired","#FF4B4B") if dleft < 0 else (
                        ("Expiring Soon","#FFA500") if dleft<=2 else ("Fresh","#4CAF50"))
        ...
        with host:
            st.markdown(f"""
                <div style='background:#1f1f1f;border-left:6px solid {color};
                  ...
                  <p style='margin:4px 0;color:#ccc;'>✅ Used: {used_count} &nbsp; | &nbsp; 🗑️ Expired: {expired_steps} &nbsp; | &nbsp; 💸 Lost: ₪{round(money_lost,2)}</p>
            ...
            with b2.expander("✏️ Edit"):
                ...
                if st.button("💾 Save", key=f"save_{item_id}"):
                    try: db_update_item(item_id, new_name.strip(), new_exp.strftime("%Y-%m-%d"), new_typ,
                                        float(new_amt), new_unit, float(new_ppu)); st.success("Saved.")
                    except Exception as e: st.error(e)
                    st.rerun()
