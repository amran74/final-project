...
def _safe_rerun():
    if hasattr(st, "rerun"):
        ...
        st.session_state["_force_refresh"] = 1 + int(st.session_state.get("_force_refresh", 0))
...
- **Cards**:
  ...
  - **View** opens details, **Cook** deducts stock (only if coverage ok), **Fav** toggles favorite.
    ...
    if section == "create":
        return """
### Create — Guide
- **Meta**: title, default servings, cook time, difficulty, course, cuisine, diet, tags, allergens, nutrition, notes.
- **Ingredients**:
  - **Mode *inventory***: pick an item already in your stock (unit auto-fills).
  - **Mode *manual***: type a name + unit; it will try to link to inventory.
  - Use **Staple/Optional** flags for cost/coverage; **Yield%** and **Waste%** adjust effective need.
- **Steps**: freeform steps with minutes + equipment; reorder with arrows.
- **Save recipe** stores to your library and computes base quantities from *qty/serv × default servings*.
        """
    if section == "ai":
        return """
### AI Create — Guide
- **Describe constraints** (e.g., “high-protein chicken dinner under 30 minutes, oven-free”).
- Press **Generate** to draft ingredients + steps. You can edit everything before saving.
- Units prefer your inventory’s base units; nutrition is auto-filled when possible.
- **Save to library** persists it like a manual recipe.
        """
    if section == "cook":
        return """
### Cook Now — Guide
- Ranks recipes by a **rescue score** combining soon-to-expire items and price.
- Shows **Coverage %** and **₪/portion**. Click **Cook** to deduct from inventory when feasible.
        """
    if section == "settings":
        return """
### Settings — Guide
- **Staples list**: global “always on hand” names (e.g., water, salt, oil). Coverage ignores shortages for staples.
- **Hard reset staples**: recompute staple flags across all recipes using the current staples list.
- **Repair links (ALL recipes)**: re-link recipe components to better-matching stocked items when available.
        """
    if section == "detail":
        return """
### Recipe Detail — Guide
- **Servings**: adjust to see coverage and cost for that amount.
- **Coverage bar**: 100% = all non-staples covered; 60–99% = partial; <60% = missing.
- **Cost**: total cost and **₪/portion** are estimated from unit prices (or total_cost ÷ amount when unit price is missing).
- **Max cookable**: the maximum number of servings you can cook right now (display only).
- **Cook now**: deducts non-staple, non-optional quantities from inventory (and will use a stocked alternative if clearly better linked).
- **Ingredients table**:
    ...
    with inner[0]:
        ...
        with st.form("builder_form", clear_on_submit=False):
            ...
            for i, step in enumerate(steps):
                ...
                if s5.form_submit_button("↑", key=f"su{i}"): _swap(steps,i,max(i-1,0)); _safe_rerun()
                if s6.form_submit_button("↓", key=f"sd{i}"): _swap(steps,i,min(i+1,len(steps)-1)); _safe_rerun()
            if st.form_submit_button("Add step ➕", key="builder_add_step"):
                _add_step(); _safe_rerun()
...
def tab_ai_create(user_id:int):
    ...
    with inner[0]:
        ...
        if not draft:
            st.info("Type constraints then click Generate.")
        else:
            with st.form("ai_edit_form", clear_on_submit=False):
                ...
                for i, step in enumerate(steps):
                    ...
                    step["minutes"] = s3.number_input("Min", min_value=0, value=int(step.get("minutes",0)), key="ai_s_min_%d"%i)
...
def _detail_ui(user_id:int, recipe_id:int):
    ...
    with c2:
        ...
        if st.session_state.get(del_key):
            st.error("This will permanently delete the recipe and all its steps/ingredients.")
            d1, d2 = st.columns(2)
            with d1:
                if st.button("Yes, delete it", key=f"del_yes_{recipe_id}"):
                    ok = rc.delete_recipe(user_id, recipe_id)
                    if ok:
    ...
    with sub[0]:
        ...
        for (_cid, ing_id, nm, bu, have, ppb, inv_type, q_base_def, is_staple, is_opt, y, wst, cat, sku, subs, note, tc) in comps:
            ...
            have_show = ci["have_base"] if ci else float(have or 0.0)
            short = ci["short_base"] if ci else 0.0
            status = ci["status"] if ci else "err"
            alt_used = bool(ci.get("alt_used", False)) if ci else False

            a,b,c,d,e,f,g = st.columns([3,2,2,2,2,1,1.2])
            a.write(nm)
            b.write(f"need {need:.1f} {bu}")
            c.write(f"have {have_show:.1f} {bu}")
            d.write("—" if short<=0 else f"short {short:.1f} {bu}")
            e.write(cat or "—")
            f.markdown(f"<span class='chip {status}'>{status}</span>", unsafe_allow_html=True)
            if alt_used:
                g.markdown("<span class='chip warn'>alt stock</span>", unsafe_allow_html=True)
            else:
                g.write("")
...
def tab_cook_now(user_id:int):
    ...
    inner = st.tabs(["Suggestions", "Guide"])
    with inner[0]:
        ...
        def rescue_score(rid:int)->float:
            sch = rc._inventory_schema()
            table = sch.get("table") or "inventory"
            exp_col = sch.get("expires") or "expiration"
            ppb_col = sch.get("price_per_base") or "price_per_base"
            amt_col = sch.get("amount") or "base_amount"
            tc_col  = sch.get("total_cost") or "total_cost"
            ...
            for r in rc.get_recipe_components(rid):
                ...
                try:
                    ...
                    c.execute(f"SELECT COALESCE({exp_col},'2099-12-31'), COALESCE({ppb_col},0), COALESCE({amt_col},0), COALESCE({tc_col},0) FROM {table} WHERE id=?", (ing_id,))
                    rr=c.fetchone()
                    conn.close()
                except Exception:
                    rr=None
                ...
                try:
                    ...
                    days=(_d.fromisoformat(rr[0])-today).days
                except Exception:
                    days=30
                urgency=max(0.0,min(1.0,(14-days)/14.0))
                need = q_base_def * (100.0+float(wst or 0))/100.0 / max(0.01, float(y or 100)/100.0)
                price = float(rr[1] or 0.0)
                if price<=0 and float(rr[3] or 0)>0 and float(rr[2] or 0)>0:
                    price=float(rr[3])/float(rr[2])
                score += urgency * price * max(0.0, float(need or 0.0))
            return score
        ...
        for r in ranked[:20]:
            ...
            if d.button("Cook", disabled=cov['status']=="err", key=f"cookk{rid}"):
                ...
                if ok:
                    ...
                    _safe_rerun()
    with inner[1]:
        st.markdown(_guide_md("cook"))

# ----------------------------- Settings tab -----------------------------------

def tab_settings(user_id:int):
    st.subheader("Settings")
    inner = st.tabs(["Preferences", "Guide"])
    with inner[0]:
        cur=cached_staples()
        with st.form("staples_form"):
            ...
            if st.form_submit_button("Save staples", key="save_staples_btn"):
                ...
                try:
                    ...
                    st.error(f"Failed to save staples: {e}")
        st.markdown("---")
    ...
    with inner[1]:
        st.markdown(_guide_md("settings"))
...
def recipes_page():
    ...
    with tabs[2]: tab_ai_create(user_id)
