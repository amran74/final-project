...
SHOP_MIGRATIONS = [
    ...
    # What each store sells (Inventory 'type')
    (
...
def run_shopping_migrations():
    ...
    for mig in SHOP_MIGRATIONS:
        ...
        try:
            ...
            # Table/column may already exist
            pass
    conn.close()
...
def get_store_categories(user_id: int, store_id: int) -> List[str]:
    ...
    c = conn.cursor()
    c.execute(
        "SELECT type FROM store_categories WHERE user_id=? AND store_id=? ORDER BY type",
        (user_id, store_id),
    )
...
def set_store_categories(user_id: int, store_id: int, types: List[str]):
    ...
    c = conn.cursor()
    c.execute(
        "DELETE FROM store_categories WHERE user_id=? AND store_id=?",
        (user_id, store_id),
    )
    for t in types:
        c.execute(
            "INSERT INTO store_categories(user_id, store_id, type) VALUES (?,?,?)",
            (user_id, store_id, t),
        )
    conn.commit()
    conn.close()
...
def add_or_update_store(
    user_id: int, name: str, kind: str, city: str, link_url: str, notes: str
):
    conn = get_connection()
    c = conn.cursor()
    c.execute(
...
def upsert_store_price(
    user_id: int, store_id: int, item_id: int, pack_qty: float, pack_unit: str, pack_price: float
):
...
def get_store_prices_for_store(user_id: int, store_id: int):
    ...
    c.execute(
      ...
      SELECT sp.item_id, i.name, i.type, i.base_unit,
             sp.pack_qty, sp.pack_unit, sp.pack_price, sp.price_per_base, sp.last_seen
...
# --------------------------------
def recent_daily_usage_base(
    user_id: int, item_id: int, base_unit: str, horizon_days: int = 60
) -> float:
...
# Smart list builder
# --------------------------------
def suggest_shopping_lines(
    user_id: int, target_days: int = 14, coverage_threshold_days: int = 7
) -> List[Dict]:
    ...
    for (
        ...
        if include and desired > 0.0:
            ...
            store_id, store_name, ppb_best = (
                bp if bp else (None, None, float(ppb or 0.0))
            )
            lines.append(
                {
                    "item_id": item_id,
...
# --------------------------------
def _insert_inventory_item_stable(
    user_id: int,
    name: str,
    typ: str,
    qty_base: float,
    ...
    """,
        (
            user_id,
            name.strip(),
            (expiration or iso_today()),
            typ,
            amount_ui,
            base_unit,  # display unit = base unit for clarity
            base_unit,
            qty_base,
            total_cost,
            price_per_base,
...
def _purchase_into_inventory(
    ...
    c = conn.cursor()
    c.execute(
        "SELECT base_amount, total_cost, price_per_base FROM inventory WHERE id=?",
        (item_id,),
    )
    row = c.fetchone()
    if not row:
    ...
    new_ppb = (
        (new_total_cost / new_base_amount)
        if new_base_amount > 0
        else (old_ppb or unit_price_base)
    )
...
def checkout_cart(user_id: int):
    ...
    for row in st.session_state.cart:
        ...
        c.execute(
          ...
          VALUES (?,?,?,?,?,?,?)
        """,
            (user_id, item_id, store_id, unit_price_base, total_paid, ts),
        )
...
def shopping():
        ...
        if stores:
            ...
            for sid, sname, skind, scity, slink, snotes in stores:
                ...
                with st.container():
                    ...
                    if slink:
                        cols[2].markdown(f"[Open site]({slink})")
                    else:
                        cols[2].write("—")
                    ...
                    if cols[4].button("Delete", key=f"del_store_{sid}"):
                        ...
                        st.rerun()
                sel = st.multiselect(
                    f"Categories for {sname}", FOOD_TYPES, default=cats, key=f"cats_{sid}"
                )
                if st.button(f"Save categories for {sname}", key=f"save_cats_{sid}"):
                    set_store_categories(user_id, sid, sel)
                    st.success("Saved")
        else:
    ...
    with st.expander("Price book (₪/base-unit) + quick buy", expanded=False):
        ...
        if not stores_all or not inv:
            ...
            store_sel = cA.selectbox(
                "Store", options=allowed_stores, format_func=lambda r: f"{r[1]} ({r[2]})", key="pb_store"
            )
            ...
            if rows:
                for it_id, nm, typ, bu, pq, pu, pp, ppb, seen in rows:
                    ...
                    buy_n = cols[3].number_input(
                        "packs", min_value=0, step=1, value=0, key=f"buy_{store_sel[0]}_{it_id}"
                    )
                    exp_line = cols[4].date_input(
                        "expiry", value=date.today(), key=f"exp_{store_sel[0]}_{it_id}"
                    )
    ...
    with st.expander("Create a NEW item here (store + price + BUY now)", expanded=False):
        ...
        if not stores_all:
            ...
            store_sel = s1.selectbox(
                "Store", options=stores_all, format_func=lambda r: f"{r[1]} ({r[2]})", key="ni_store"
            )
            ...
            if cats and typ not in cats:
                st.warning(
                    f"⚠️ {store_sel[1]} is not marked as selling '{typ}'. "
                    f"You can still proceed, or add the category above."
                )
            ...
            if st.button("Create item as Stable + save price + BUY"):
                if not name.strip():
                    ...
                    base_per_pack, _ = to_base(pack_qty, pack_unit)
                    ...
                    item_id = _insert_inventory_item_stable(
                        ...
                        typ=typ,
                        qty_base=qty_base,
                        base_unit=BASE_FOR.get(base_unit, "pcs"),
                        price_per_base=unit_price_base,
                    ...
                    # Add a zero-qty purchase entry to record the source store & price today
                    _purchase_into_inventory(
                        user_id=user_id,
                        item_id=item_id,
                        qty_base=0.0,
    ...
    with st.expander("Stock rules (Always-buy & Par levels)", expanded=False):
        ...
        if not inv:
            ...
            for (iid, nm, typ, _kind, bu, onhand, _ppb, always_buy, par, _stable) in inv:
                ...
                new_par = cols[2].number_input(
                    "Par (base)", min_value=0.0, step=step_size_for(bu), value=float(par or 0.0), key=f"par_{iid}"
                )
                if cols[3].button("Save", key=f"sv_{iid}"):
                    set_item_rules(iid, ab, new_par)
                    st.success("Rules saved")

    # --- MAP STABLE ITEMS TO STORES ---------------------------------------
    with st.expander("Map stable items to stores (add prices fast)", expanded=False):
        inv_stable = [r for r in get_inventory(user_id) if r[-1] == 1]  # stable==1
        if not inv_stable:
            ...
            irow = left.selectbox(
                "Stable item",
                options=inv_stable,
                format_func=lambda r: f"{r[1]} [{r[2]} · {r[4]}]",
            )
            stores_ok = stores_selling_type(user_id, irow[2]) or get_stores(user_id)
            srow = left.selectbox(
                "Store", options=stores_ok, format_func=lambda r: f"{r[1]} ({r[2]})", key="map_store"
            )
    ...
    if "suggestions" in st.session_state and st.session_state.suggestions:
        for idx, row in enumerate(st.session_state.suggestions):
            ...
            chosen = c3.selectbox(
                "Store",
                options=[(None, "—", "", "", "")] + stores_ok,
                index=0
                if store_id is None
                else 1 + next((i for i, s in enumerate(stores_ok) if s[0] == store_id), 0),
            ...
            unit_price = c4.number_input(
                ...
                min_value=0.0,
                step=0.1,
                value=float(row["unit_price_base"] or 0.0),
                key=f"sugg_ppb_{idx}",
            )
    ...
    with st.expander("Quick add to cart", expanded=False):
        ...
        if inv:
            irow = st.selectbox(
                "Item", options=inv, format_func=lambda r: f"{r[1]} [{r[2]} · {r[4]}]"
            )
    ...
    if not st.session_state.cart:
        ...
        for idx, row in enumerate(st.session_state.cart):
            ...
            row["unit_price_base"] = c3.number_input(
                "₪/base",
                min_value=0.0,
                step=0.1,
            ...
            cur_index = (
                ...
                else 1 + next((i for i, s in enumerate(stores) if s[0] == row["store_id"]), 0)
            )
            chosen = c4.selectbox(
                "Store",
                options=store_opts,
        ...
        for sid, total in totals_by_store.items():
            store_name = (
                "No store"
                if sid is None
                else next((s[1] for s in get_stores(user_id) if s[0] == sid), "Store")
            )
    ...
    with st.expander("Save / Load lists", expanded=False):
        ...
        if st.button("Save current cart as template"):
            if not st.session_state.cart:
                ...
                for row in st.session_state.cart:
                    c.execute(
                        """
                      INSERT INTO shopping_list_items(list_id, item_id, desired_qty_base,
                                                     chosen_store_id, est_unit_price_base, est_total, expiration)
                      VALUES (?,?,?,?,?,?,?)
                    """,
                        (
                            list_id,
        ...
        c = conn.cursor()
        c.execute(
            "SELECT id, title, created_at FROM shopping_lists WHERE user_id=? AND status='template' ORDER BY created_at DESC",
            (user_id,),
        )
        ...
        if templates:
            chosen = st.selectbox(
                "Load template", options=templates, format_func=lambda r: f"{r[1]} ({r[2]})"
            )
