...
def to_base(amount: float, unit: str) -> Tuple[float, str]:
    ...
    if unit not in BASE_FOR:  # fallback
...
def suggest_shopping_lines(user_id: int,
    ...
    """
    Produce suggestions:
    - Include all always_buy items (qty = max(par-onhand, one step))
    - Include items with < coverage_threshold_days of stock based on recent usage
    - Choose best store by price book when possible
    """
    ...
    for (item_id, name, typ, kind, base_unit, on_hand, ppb, always_buy, par_base) in inv:
        ...
        avg_daily = recent_daily_usage_base(user_id, item_id, base_unit)  # base units / day
        ...
        if always_buy:
            include = True
            if par_base > 0:
                desired = max(par_base - on_hand, 0.0)
            if desired <= 0:
                desired = step_size_for(base_unit)  # at least one step
...
def _ensure_cart():
    if "cart" not in st.session_state:
        st.session_state.cart = []  # list of dict rows as above
...
def checkout_cart(user_id: int):
    ...
    for row in st.session_state.cart:
        ...
        # Touch price book last_seen for the store/price (if any)
    ...
    with st.expander("Stores", expanded=False):
        ...
        if stores:
            ...
            for sid, sname, skind, scity, slink, snotes in stores:
                ...
                if slink:
                    cols[3].markdown(f"[Open site]({slink})")
                else:
                    cols[3].write("—")
